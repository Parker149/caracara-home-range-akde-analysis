...
# READ ME: Run this loop if you want to plot the home ranges with the NLCD raster
for (i in seq_along(hr_list)) {
  ...
  plot(hr["geometry"], add = TRUE, border = "black", lwd = 2)
  legend(x = 520000, y = 3119200,,
         legend = nlcd_names,
         fill = nlcd_cols,
         bty = "o",
         cex = 0.7,
...
# READ ME: Run this loop if you want to save the above plots as pngs
setwd("/Users/parkerwilkerson/Desktop/EEB_HW/Caracara/figures/")
for (i in seq_along(hr_list)) {
  fname <- paste0(names(hr_list)[i], "_seasonal_HR_NLCD.png")
  png(filename = fname,
      width = 5,
      height = 7,
      units = "in",
...
for (i in seq_along(hr_list)) {
  hr <- hr_list[[i]]
  vals <- terra::extract(NLCD, hr)
  vals_vec <- unlist(vals[,-1])
  results <- prop.table(table(vals_vec)) * 100
  results_full <- setNames(rep(0, length(nlcd_vals)), nlcd_vals)
  results_full[names(results)] <- results
  # Pie chart
  pie(results_full,
      labels = NA,
      col = nlcd_cols,
  ...
  print(data.frame(
    ...
    Percent = round(as.numeric(results_full), 2)
  ))
}
...
# --- Run this to save pie charts ---
setwd("/Users/parkerwilkerson/Desktop/EEB_HW/Caracara/figures/")
for (i in seq_along(hr_list)) {
  fname <- paste0(names(hr_list)[i], "_pie_NLCD.png")
  png(filename = fname,
      width = 7,
      height = 7,
      units = "in",
